 ...
 */
void allocFreeTest( SimpleAllocator* allocator, unsigned numObjsToAllocate = 0, unsigned numObjsToFree = 0, bool maskExternalHeaders = false) 
{
  try {
    ...
    for (unsigned i = 0; i < numObjsToAllocate; i++) 
    {
        ...
        // - also concat the label by adding the prefix "stu"
        // - convert the label to a c-string using std::to_string
        // - finally, pass the c-string to the allocator
        ptrs[i] = allocator->allocate(("stu" + std::to_string(randInt(10000, 99999))).c_str());
...
int main(int argc, char *argv[]) {
  ...
  case 7:
    ...
    // create the allocator
    allocator = createAllocator(false, 4, 8,  SimpleAllocatorConfig::EXTENDED_HEADER, 0, 0, true, TestObjectType::STUDENT_TYPE);
